...
4. An async Action Scheduler action sends the email in the background (a bypass flag prevents re-interception).
5. Failures retry up to 3 times with backoff, then are marked failed and visible under **Tools → Mail Queue** with a "Send again" action.
...
- Health warnings on the admin page for a stuck queue or a foreign pluggable `wp_mail()` override (which would make interception impossible).
